...
public class BaseClass {
    ...
    public void SwichSecondWindow(){
        List<String> strings = driver.getWindowHandles().stream().toList();
        driver.switchTo().window(strings.get(1));
    }

    public static boolean isVisibleElement(String formatLink){

        By linkLocator = By.xpath(MessageFormat.format("//*[text()=\"{0}\"]", formatLink));
        try {
            new WebDriverWait(getDriver(),Duration.ofMillis(10000)).until(ExpectedConditions.visibilityOfElementLocated(linkLocator));
            return true;
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
